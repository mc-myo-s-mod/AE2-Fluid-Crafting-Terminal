...
public class FluidCraftingConfig {
    ...
    public static boolean isVirtualFluidRecipeBlacklisted(ResourceLocation recipeId) {
        return isVirtualFluidRecipeBlacklisted(recipeId, COMMON.virtualFluidRecipeBlacklist.get());
    }

    public static boolean isVirtualFluidRecipeBlacklisted(ResourceLocation recipeId, Iterable<? extends String> recipeIds) {
        if (recipeId == null) {
            return false;
        }

        String recipeIdString = recipeId.toString();
        for (String blacklistedId : recipeIds) {
            if (recipeIdString.equals(blacklistedId)) {
                return true;
            }
        }
        return false;
    ...
    public static String encodeVirtualFluidRecipeBlacklist() {
        return String.join("\n", COMMON.virtualFluidRecipeBlacklist.get());
    }

    public static boolean isVirtualFluidRecipeBlacklisted(ResourceLocation recipeId, String encodedRecipeIds) {
        if (encodedRecipeIds == null || encodedRecipeIds.isBlank()) {
            return false;
        }

        return isVirtualFluidRecipeBlacklisted(recipeId, List.of(encodedRecipeIds.split("\n")));
    }
